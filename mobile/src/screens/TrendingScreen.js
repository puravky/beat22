...
import { fetchAllBeats } from '../api/client';
...
export default function TrendingScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setBeats([...data].sort((a, b) => b.plays - a.plays).slice(0, 20));
    } catch (err) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <Animated.View style={{ flex: 1, opacity: fadeAnim }}>
        ...
        <Text style={styles.subtitle}>Most played beats right now</Text>
        ...
        <EmptyState title="Nothing trending" subtitle="Check back soon." />
      ) : (
        <FlatList
          data={beats}
